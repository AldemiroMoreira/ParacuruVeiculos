const LoginPage = ({ onLogin, navigateTo }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // onLogin handles state update and navigation
            onLogin(user);
        } catch (err) {
            console.error('Login error:', err);
            const message = err.response?.data?.error || err.message || 'Erro ao conectar ao servidor. Tente novamente.';
            setError(message);
        }
    };
    ...
    return (
        <div className="max-w-xs mx-auto mt-10 bg-white p-5 rounded-xl shadow-lg border border-gray-100 fade-in">
            <h2 className="text-xl font-bold text-center mb-4 text-gray-900">Bem-vindo de volta</h2>
                ...
                <div>
                    ...
                    <div className="relative">
                        <input
                            ...
                            className="w-full p-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:outline-none text-sm pr-10"
